...
def process_block(args):
    ...
    occupied = [[[False for _ in range(res)] for _ in range(res)] for _ in range(res)]
    for i in range(res):
        for j in range(res):
            for k in range(res):
                center = [block_min[0] + (i + 0.5) * voxel_size, block_min[1] + (j + 0.5) * voxel_size, block_min[2] + (k + 0.5) * voxel_size]
                for off in offsets:
                    point = add(center, off)
                    if is_inside(point, triangles, is_watertight):
                        occupied[i][j][k] = True
                        break
    occupied_np = np.array(occupied)
    if global_postprocess:
...
def parse_bbmodel(file_path, x_threshold, y_threshold, z_threshold, no_postprocess, no_holes, no_gaps, no_small_voids, gap_passes, small_void_threshold, global_postprocess, per_block_gap_x=False, per_block_gap_y=False, per_block_gap_z=False):
    ...
    with mp.Pool() as pool:
        results = list(tqdm(pool.imap_unordered(process_block, args_list), total=len(args_list), desc=os.path.basename(file_path)))
    aabbs = [r for r in results if r is not None]
    if global_postprocess:
