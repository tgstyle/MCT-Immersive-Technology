...
def ray_triangle_intersect(origin, direction, v0, v1, v2, epsilon):
    ...
    return False
def is_inside(point, triangles, is_watertight):
    if is_watertight:
    ...
    for direction in directions:
        ...
        for v0, v1, v2 in triangles:
            if ray_triangle_intersect(point, direction, v0, v1, v2, eps): count += 1
        if count % 2 == 1: inside_count += 1
    return inside_count >= required
...
def process_block(args):
    bx, by, bz, minx, miny, minz, verts, triangles, edges, res, x_threshold, y_threshold, z_threshold, is_watertight, has_thin_features, no_postprocess, no_holes, no_gaps, no_small_voids, gap_passes, small_void_threshold, global_postprocess = args
    ...
    if not has_intersection:
        ...
        if is_inside(block_center, triangles, is_watertight):
            if global_postprocess:
                # Return full voxel grid for nearly full block
    ...
    offsets = [
        [0, 0, 0], [delta, 0, 0], [-delta, 0, 0], [0, delta, 0], [0, -delta, 0], [0, 0, delta], [0, 0, -delta],
    ...
    ] if has_thin_features else [[0, 0, 0], [delta, 0, 0], [-delta, 0, 0], [0, delta, 0], [0, -delta, 0], [0, 0, delta], [0, 0, -delta]]
    ...
    if is_watertight:
        ...
        required = 3
        epsilon = -1e-6
    directions_t = torch.tensor(directions, dtype=torch.float32, device=device)
    num_dir = len(directions)
    num_off = len(offsets)
    num_vox = res ** 3
    i = torch.arange(res, dtype=torch.float32, device=device)
    ii, jj, kk = torch.meshgrid(i, i, i, indexing='ij')
    centers = torch.stack([
        torch.tensor(block_min[0], device=device) + (ii + 0.5) * voxel_size,
        torch.tensor(block_min[1], device=device) + (jj + 0.5) * voxel_size,
        torch.tensor(block_min[2], device=device) + (kk + 0.5) * voxel_size
    ], dim=-1) # (res, res, res, 3)
    centers_flat = centers.view(-1, 3) # (num_vox, 3)
    all_points = centers_flat[:, None, :] + offsets_t[None, :, :] # (num_vox, num_off, 3)
    all_points_flat = all_points.view(-1, 3) # (num_vox * num_off, 3)
    num_points = len(all_points_flat)
    all_origins = all_points_flat[:, None, :].repeat(1, num_dir, 1).view(-1, 3) # (num_points * num_dir, 3)
    all_dirs = directions_t[None, :, :].repeat(num_points, 1, 1).view(-1, 3) # (num_points * num_dir, 3)
    num_rays = len(all_origins)
    T = len(triangles)
    if T == 0:
        inside = torch.zeros(num_points, dtype=torch.bool, device=device)
    else:
        tris_t = torch.tensor(triangles, dtype=torch.float32, device=device) # (T, 3, 3)
        v0 = tris_t[:, 0, :]
        v1 = tris_t[:, 1, :]
        v2 = tris_t[:, 2, :]
        edge1 = v1 - v0 # (T, 3)
        ...
        for start in range(0, num_rays, batch_size):
            ...
            o_batch = all_origins[start:end, None, :] # (B, 1, 3)
            d_batch = all_dirs[start:end, None, :] # (B, 1, 3)
            h = torch.stack([
        ...
        odd = (counts % 2) == 1
        odd_per_dir = odd.view(num_points, num_dir) # (num_points, num_dir)
        inside_dirs = odd_per_dir.sum(dim=1, dtype=torch.int32) # (num_points,)
        inside = inside_dirs >= required # (num_points,)
    inside_per_vox = inside.view(num_vox, num_off).any(dim=1) # (num_vox,)
    occupied_np = inside_per_vox.view(res, res, res).cpu().numpy().astype(bool)
    if global_postprocess:
        # Return voxel grid for global post-processing
        return (bx, by, bz, occupied_np)
    if no_postprocess:
        pass
    else:
    ...
    return None
def parse_bbmodel(file_path, x_threshold, y_threshold, z_threshold, no_postprocess, no_holes, no_gaps, no_small_voids, gap_passes, small_void_threshold, global_postprocess, per_block_gap_x=False, per_block_gap_y=False, per_block_gap_z=False):
...
def main():
    ...
    parser.add_argument('--small-void-threshold', type=int, default=4, help='Maximum voxel count for small voids to fill (fills if size < threshold)')
    parser.add_argument('--no-global-postprocess', action='store_true', help='Disable global post-processing on the full model (default: enabled)')
    ...
    parser.add_argument('--per-block-gap-x', action='store_true', help='Force per-block gap filling for X axis when global postprocess is enabled')
    parser.add_argument('--per-block-gap-y', action='store_true', help='Force per-block gap filling for Y axis when global postprocess is enabled')
    parser.add_argument('--per-block-gap-z', action='store_true', help='Force per-block gap filling for Z axis when global postprocess is enabled')
    args = parser.parse_args()
    global_postprocess = not args.no_global_postprocess
    main_path = args.path
    directory = '.'
    if main_path:
        if os.path.isfile(main_path) and main_path.lower().endswith('.bbmodel'):
            directory = os.path.dirname(main_path) or '.'
        elif os.path.isdir(main_path):
            directory = main_path
            main_path = None
        else:
            print(f"Invalid path: {main_path}. Falling back to current directory.")
    bbmodel_files = list_bbmodel_files(directory)
    if main_path:
        main_file = os.path.basename(main_path)
        if main_file not in bbmodel_files: raise ValueError("Specified main file not found in directory.")
    ...
    overall_aabbs = parse_bbmodel(main_path, args.x_threshold, args.y_threshold, args.z_threshold, args.no_postprocess, args.no_holes, args.no_gaps, args.no_small_voids, args.gap_passes, args.small_void_threshold, global_postprocess, args.per_block_gap_x, args.per_block_gap_y, args.per_block_gap_z)
    ...
    for s_file in unique_supps:
        ...
        s_aabbs = parse_bbmodel(s_path, args.x_threshold, args.y_threshold, args.z_threshold, args.no_postprocess, args.no_holes, args.no_gaps, args.no_small_voids, args.gap_passes, args.small_void_threshold, global_postprocess, args.per_block_gap_x, args.per_block_gap_y, args.per_block_gap_z)
