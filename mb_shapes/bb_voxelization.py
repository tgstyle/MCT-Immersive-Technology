...
def process_block(args):
    ...
    for i1, i2 in edges:
        ...
        for dim in range(3):
            for side in [block_min[dim], block_max[dim]]:
                ...
                t_param = (side - v1[dim]) / denom
                if 0 <= t_param <= 1:
                    inter = [v1[k] + t_param * (v2[k] - v1[k]) for k in range(3)]
                    if all(block_min[k] <= inter[k] < block_max[k] for k in range(3) if k != dim):
                        points.append(inter)
                        has_intersection = True

    # Prepare voxel centers
    voxel_size = 16 / res
    ...
    # Set up ray directions based on model properties
    if is_watertight:
    ...
    if t == 0:
        ...
        # Batch processing for ray-triangle intersection
        ...
        with torch.no_grad():
            ...
            if t * num_rays < full_try_threshold:
                try:
                    # Full batch computation if possible
                    v0_b = v0[None, :, :]
                    edge1_b = edge1[None, :, :]
                    edge2_b = edge2[None, :, :]
                    ...
                    mask &= (v >= 0.0) & (u + v <= 1.0)
                    t_param = f * torch.einsum('btj,btj->bt', edge2_b, q)
                    mask &= (t_param > epsilon)
                    intersects = mask
                    counts = intersects.sum(dim=1, dtype=torch.int32)
                    del o_batch, d_batch, h, a, mask, f, s, u, q, v, t_param, intersects, v0_b, edge1_b, edge2_b
    ...
    # Aggregate inside status per voxel
    ...
    # Handle non-intersecting blocks
