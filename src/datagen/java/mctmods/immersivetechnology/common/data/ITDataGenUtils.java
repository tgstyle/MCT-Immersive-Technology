...
public class ITDataGenUtils {
    ...
    public static String getTextureFromObj(ResourceLocation obj, ExistingFileHelper helper) {
        try {
            ...
            if (obj.getPath().startsWith("models/")) { prefix = ""; }
    ...
    public static String getMTLTexture(ResourceLocation mtl, String materialName, ExistingFileHelper helper) {
        try {
            ...
            while (materialMatcher.find()) {
                if (materialMatcher.group(1).equals(materialName)) {
                    found = true;
                    break;
                }
            }
            if (!found) { throw new RuntimeException("Material not found: " + materialName); }
            int materialStart = materialMatcher.start();
            int materialEnd;
            if (materialMatcher.find()) { materialEnd = materialMatcher.start(); }
            else { materialEnd = fullMtl.length(); }
