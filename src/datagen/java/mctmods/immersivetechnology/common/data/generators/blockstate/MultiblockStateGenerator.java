...
public class MultiblockStateGenerator {
    ...
    private ModelFile split(ITNongeneratedModel unsplit, ModTemplateMultiblock multiblock, UnaryOperator<BlockPos> transform, String block_type) {
        loadTemplateFor(multiblock);
    ...
    protected void addParticleTextureFrom(BlockModelBuilder result, ModelBuilder<?> model) {
        ...
        if (particles != null) { result.texture("particle", particles); ModBlockState.generatedParticleTextures.put(result.getLocation(), particles); }
    }

    private void loadTemplateFor(ModTemplateMultiblock multiblock) {
        final ResourceLocation name = multiblock.getUniqueName();
        if (ModTemplateMultiblock.SYNCED_CLIENT_TEMPLATES.containsKey(name)) {
            return;
        }
        Reference.IT_LOGGER.info("Loading structure template for {} during datagen", name);
    ...
    private void createMultiblockVariant(Supplier<? extends Block> b, ModelFile defaultMaster, @Nullable ModelFile activeMaster, @Nullable ModelFile defaultMirrored, @Nullable ModelFile activeMirrored, @Nullable Property<Boolean> mirroredState, @Nullable Property<Boolean> activeState) {
        ...
        Reference.IT_LOGGER.info("Registered blockstate for multiblock: {}", BuiltInRegistries.BLOCK.getKey(b.get()));
        StateDefinition<?, ?> def = b.get().getStateDefinition();
        EnumProperty<Direction> facing = def.getProperties().contains(ModProperties.FACING_HORIZONTAL) ? ModProperties.FACING_HORIZONTAL : null;
        try {
