...
public class MultiblockCokeOvenAdvanced implements IMultiblock{
	...
	public boolean isBlockTrigger(IBlockState state) {
		return state.getBlock()==ITContent.blockStoneDecoration && 
				(state.getBlock().getMetaFromState(state)==BlockType_StoneDecoration.COKEBRICK_REINFORCED.getMeta());
	}
	...
	public boolean createStructure(World world, BlockPos pos, EnumFacing side, EntityPlayer player) {
		...
		if(!structureCheck(world, pos, side)){
			return false;
		}
				...
				for(int w=-1;w<=1;w++){
					...
					if(h==2) {
						...
						if(l==2) {
							...
							if(!Utils.isBlockAt(world, pos, ITContent.blockStoneDecoration, BlockType_StoneDecoration.COKEBRICK_REINFORCED.getMeta())){
								//System.out.println("ERROR AT: h "+h+", l "+l+", w "+w);
								return false;
							}
						}
	...
	public void renderFormedStructure() {
		if(renderStack.isEmpty())
			renderStack = new ItemStack(ITContent.blockStoneMultiblock,1,BlockType_StoneMultiblock.COKE_OVEN_ADVANCED.getMeta());
