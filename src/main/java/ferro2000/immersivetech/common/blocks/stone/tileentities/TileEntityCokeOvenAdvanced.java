...
public class TileEntityCokeOvenAdvanced extends TileEntityMultiblockPart<TileEntityCokeOvenAdvanced>
		implements IIEInventory, IActiveState, IGuiTile, IProcessTile, IAdvancedSelectionBounds, IAdvancedCollisionBounds {
		...
		if(!world.isRemote&&formed&&!isDummy())
		{
			...
			if(!this.inventory.get(1).isEmpty())
			{
				...
				if(inventoryFront!=null)
					stack = Utils.insertStackIntoInventory(inventoryFront, stack, facing);
	...
	private int getProcessSpeed() {
		...
		for(int k=0; k<2; k++) {
			...
			if(tilePreheater instanceof TileEntityCokeOvenPreheater) {
				
				if(((TileEntityCokeOvenPreheater) tilePreheater).facing == f.getOpposite()) {
					
					i += ((TileEntityCokeOvenPreheater) tilePreheater).doSpeedup();
