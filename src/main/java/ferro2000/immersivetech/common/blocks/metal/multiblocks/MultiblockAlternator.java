...
public class MultiblockAlternator implements IMultiblock {
			...
			for(int l=0;l<=3;l++) {
				for(int w=-1;w<=1;w++) {
					...
					if(h==0 && w==0 && l>1) {
						...
						if(!Utils.isBlockAt(world, pos, IEContent.blockMetalDevice0, BlockTypes_MetalDevice0.CAPACITOR_HV.getMeta())){
							return false;
						}
					}else {
	...
	static final IngredientStack[] materials = new IngredientStack[]{
			...
			new IngredientStack(new ItemStack(IEContent.blockMetalDecoration0, 5, BlockTypes_MetalDevice0.CAPACITOR_HV.getMeta())),
	...
	public void renderFormedStructure() {
		if(renderStack==null)
			renderStack = new ItemStack(ITContent.blockMetalMultiblock,1,BlockType_MetalMultiblock.ALTERNATOR.getMeta());
