...
public class TileEntitySolarTower extends TileEntityMultiblockMetal<TileEntitySolarTower, SolarTowerRecipes>
		implements IGuiTile, IAdvancedSelectionBounds, IAdvancedCollisionBounds {
			...
			if (tanks[0].getFluidAmount() > 0) {
				...
				if (recipe != null) {
					...
					if (this.processTime > 30) {
						this.processTime = 0;
						MultiblockProcessInMachine<SolarTowerRecipes> process = new MultiblockProcessInMachine(recipe)
			...
			for (int i = 1; i < maxRange + 2; i++) {
				...
				if (!Utils.isBlockAt(world, pos, Blocks.AIR, 0)) {
					...
					if (tile instanceof TileEntitySolarReflector) {
						...
						if ((cont % 2 == 0 && (facing == EnumFacing.NORTH || facing == EnumFacing.SOUTH))
								|| (cont % 2 != 0 && (facing == EnumFacing.EAST || facing == EnumFacing.WEST))) {
	...
	public List<AxisAlignedBB> getAdvancedSelectionBounds() {
		...
		if (pos == 0 || pos == 2 || pos == 6 || pos == 8 || pos == 62 || pos == 60 || pos == 56 || pos == 54) {
			...
			float minX = fl == EnumFacing.WEST ? .6875f
					: fl == EnumFacing.EAST ? .0625f : fw == EnumFacing.EAST ? .0625f : .6875f;
			float maxX = fl == EnumFacing.EAST ? .3125f
					: fl == EnumFacing.WEST ? .9375f : fw == EnumFacing.EAST ? .3125f : .9375f;
			float minZ = fl == EnumFacing.NORTH ? .6875f
					: fl == EnumFacing.SOUTH ? .0625f : fw == EnumFacing.SOUTH ? .0625f : .6875f;
			float maxZ = fl == EnumFacing.SOUTH ? .3125f
					: fl == EnumFacing.NORTH ? .9375f : fw == EnumFacing.SOUTH ? .3125f : .9375f;
			minY = .5f;
			maxY = 1;
			if (pos > 8) {
				minY = 0;
			...
			list.add(new AxisAlignedBB(minX, minY, minZ, maxX, maxY, maxZ).offset(getPos().getX(), getPos().getY(),
					getPos().getZ()));
			...
			list.add(new AxisAlignedBB(minX, minY, 0, maxX, maxY, 1).offset(getPos().getX(), getPos().getY(),
					getPos().getZ()));
			...
			list.add(new AxisAlignedBB(0, minY, minZ, 1, maxY, maxZ).offset(getPos().getX(), getPos().getY(),
					getPos().getZ()));
		...
		if ((pos != 0 && pos != 54 && (pos % 9 == 0))) {
			...
			list.add(new AxisAlignedBB(minX, 0, 0, maxX, 1, 1).offset(getPos().getX(), getPos().getY(),
					getPos().getZ()));

			minZ = fl == EnumFacing.NORTH ? 0 : fl == EnumFacing.SOUTH ? .5f : fw == EnumFacing.NORTH ? 0 : .5f;
			maxZ = fl == EnumFacing.NORTH ? .5f : fl == EnumFacing.SOUTH ? 1 : fw == EnumFacing.NORTH ? .5f : 1;

			list.add(new AxisAlignedBB(0, 0, minZ, 1, 1, maxZ).offset(getPos().getX(), getPos().getY(),
					getPos().getZ()));
