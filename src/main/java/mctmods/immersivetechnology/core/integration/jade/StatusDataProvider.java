...
public enum StatusDataProvider implements IServerDataProvider<BlockAccessor> {
    INSTANCE;
    ...
    @Override public ResourceLocation getUid() { return Reference.rl("status"); }
    ...
    @Override public void appendServerData(CompoundTag data, BlockAccessor accessor) {
        if (accessor.getBlockEntity() instanceof IMultiblockBE<?> mbe) {
            ...
            if (state instanceof AdvancedCokeOvenLogic.State coke) { active = coke.isActive(); }
            ...
            else if (state instanceof DistillerLogic.State distiller) { active = distiller.isActive(); }
