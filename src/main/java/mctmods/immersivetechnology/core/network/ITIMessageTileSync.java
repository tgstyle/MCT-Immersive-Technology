...
public class ITIMessageTileSync implements ITIMessage {
    private final BlockPos pos;
    private final CompoundTag nbt;

    public ITIMessageTileSync(BlockPos pos, CompoundTag nbt) {
        this.pos = pos;
        this.nbt = nbt;
    }

    public ITIMessageTileSync(FriendlyByteBuf buf) {
    ...
    @Override public void process(Supplier<NetworkEvent.Context> context) {
        ...
        if (player != null) {
            ctx.enqueueWork(() -> {
                ...
                if (tile instanceof ITIBaseIBlockEntity itbe) {
