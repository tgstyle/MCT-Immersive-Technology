package mctmods.immersivetechnology.common.blocks.gui.helper;
...
public abstract class ITContainerMenu extends AbstractContainerMenu {
    ...
    protected ITContainerMenu(MenuContext ctx) { super(ctx.type,ctx.id); this.setChanged=ctx.setChanged; this.isValid=ctx.isValid; }
    public void addGenericData(GenericContainerData<?> newData) { genericData.add(newData); }
    @Override public void broadcastChanges() {
        super.broadcastChanges();
        List<Pair<Integer,DataPair<?>>> toSync=new ArrayList<>();
        for (int i=0;i<genericData.size();i++) { GenericContainerData<?> data=genericData.get(i); if (data.needsUpdate()) toSync.add(Pair.of(i,data.dataPair())); }
    ...
    public void receiveSync(List<Pair<Integer,DataPair<?>>> synced) { for (Pair<Integer,DataPair<?>> syncElement:synced) genericData.get(syncElement.getFirst()).processSync(syncElement.getSecond().data()); }
    ...
    @Nonnull @Override public ItemStack quickMoveStack(@NotNull Player player,int slot) {
        ItemStack itemStack=ItemStack.EMPTY;
        Slot slotObject=this.slots.get(slot);
        if (slotObject.hasItem()) {
            ItemStack itemStack1=slotObject.getItem();
            itemStack=itemStack1.copy();
            if (slot<ownSlotCount) { if (!this.moveItemStackTo(itemStack1,ownSlotCount,this.slots.size(),true)) return ItemStack.EMPTY; }
    ...
    public static boolean moveItemStackToWithMayPlace(List<Slot> slots,MoveItemsFunc move,ItemStack pStack,int pStartIndex,int pEndIndex) {
        ...
        for (int i=pStartIndex;i<pEndIndex;i++) {
            ...
            if (inAllowedRange&&!mayPlace) { if (move.moveItemStackTo(pStack,allowedStart,i,false)) return true; inAllowedRange=false; }
            else if (!inAllowedRange&&mayPlace) { allowedStart=i; inAllowedRange=true; }
        }
    ...
    @SubscribeEvent public static void onContainerOpened(PlayerContainerEvent.Open ev) {
        if (ev.getContainer() instanceof ITContainerMenu itContainer&&ev.getEntity() instanceof ServerPlayer serverPlayer) {
            ...
            for (int i=0;i<itContainer.genericData.size();i++) list.add(Pair.of(i,itContainer.genericData.get(i).dataPair()));
            ITPacketHandler.sendToPlayer(serverPlayer,new ITMessageContainerData(list));
        }
    }
    @SubscribeEvent public static void onContainerClosed(PlayerContainerEvent.Close ev) { if (ev.getContainer() instanceof ITContainerMenu itContainer&&ev.getEntity() instanceof ServerPlayer serverPlayer) itContainer.usingPlayers.remove(serverPlayer); }
    public static MenuContext multiblockCtx(MenuType<?> pMenuType,int pContainerId,MultiblockMenuContext<?> ctx) { return new MenuContext(pMenuType,pContainerId,ctx.mbContext()::markMasterDirty,p->{ if (!ctx.mbContext().isValid().getAsBoolean()) return false; return p.distanceToSqr(Vec3.atCenterOf(ctx.clickedPos))<=64.0D; }); }
    public static MenuContext blockCtx(MenuType<?> pMenuType, int pContainerId, BlockEntity be) {
        return new MenuContext(pMenuType,pContainerId,()->{ be.setChanged(); if (be instanceof ITBaseBlockEntity itBE) itBE.markContainingBlockForUpdate(null); },p->{
            BlockPos pos=be.getBlockPos();
            Level level=be.getLevel();
            if (level==null||level.getBlockEntity(pos)!=be) return false; else return !(p.distanceToSqr(pos.getX()+0.5D,pos.getY()+0.5D,pos.getZ()+0.5D)>64.0D);
        });
    }
    ...
    public interface MoveItemsFunc { boolean moveItemStackTo(ItemStack stack,int startIndex,int endIndex,boolean reverseDirection); }
}
