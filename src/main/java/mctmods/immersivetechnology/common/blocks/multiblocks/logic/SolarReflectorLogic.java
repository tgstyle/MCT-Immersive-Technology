...
public class SolarReflectorLogic implements IMultiblockLogic<SolarReflectorLogic.State>, IServerTickableComponent<SolarReflectorLogic.State>, IClientTickableComponent<SolarReflectorLogic.State> {
    ...
    @Override
    public LazyOptional getCapability(IMultiblockContext ctx, CapabilityPosition position, Capability cap) { return IMultiblockLogic.super.getCapability(ctx, position, cap); }

    @Override
    public InteractionResult click(IMultiblockContext ctx, BlockPos posInMultiblock, Player player, InteractionHand hand, BlockHitResult absoluteHit, boolean isClient) { return IMultiblockLogic.super.click(ctx, posInMultiblock, player, hand, absoluteHit, isClient); }
    ...
    public static class State implements IMultiblockState, ProcessContext.ProcessContextInWorld<SolarTowerRecipe> {
        ...
        @Override
        public void writeSyncNBT(CompoundTag nbt) { IMultiblockState.super.writeSyncNBT(nbt); }

        @Override
        public void readSaveNBT(CompoundTag compoundTag) { }

        @Override
        public void readSyncNBT(CompoundTag nbt) { IMultiblockState.super.readSyncNBT(nbt); }
        ...
        @Override
        public IFluidTank[] getInternalTanks() { return ProcessContextInWorld.super.getInternalTanks(); }

        @Override
        public void doProcessFluidOutput(FluidStack output) { ProcessContextInWorld.super.doProcessFluidOutput(output); }
    }
}
