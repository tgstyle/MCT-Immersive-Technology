...
public class SolarTowerProcess extends MultiblockProcessInMachine<SolarTowerRecipe> {
    public SolarTowerProcess(SolarTowerRecipe recipe) {
        super(recipe);
        this.setInputTanks(0);
    }

    public SolarTowerProcess(BiFunction<Level, ResourceLocation, SolarTowerRecipe> getRecipe, CompoundTag data) {
    ...
    public void doProcessTick(ProcessContext.ProcessContextInMachine<SolarTowerRecipe> context, IMultiblockLevel level) {
        ...
        if (state.heatLevel < SolarTowerLogic.WORKING_HEAT_LEVEL) {
            this.processTick = Math.max(this.processTick - SolarTowerLogic.PROGRESS_LOSS_PER_TICK, 0);
            return;
        }
