...
public class BoilerProcess implements IMultiblockComponent<ITBoilerLogic.State> {
    private static final BlockPos IGNITION_POS = new BlockPos(4, 1, 1);

    @Override
    public InteractionResult click(IMultiblockContext<ITBoilerLogic.State> ctx, BlockPos posInMultiblock, Player player, InteractionHand hand, BlockHitResult absoluteHit, boolean isClient) {
        ...
        ITBoilerLogic.State state = ctx.getState();
        ...
        state.pilotLit = true;
        state.heatLevel = ITBoilerLogic.PILOT_HEAT;
