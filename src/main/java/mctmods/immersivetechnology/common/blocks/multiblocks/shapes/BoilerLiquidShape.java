...
public class BoilerLiquidShape extends GenericShape {
    ...
    private BoilerLiquidShape(List<List<AABB>> shapes) { this.SHAPES = shapes; }

    static {
        int[] dims = loadDimensions("boiler_liquid", "metal");
        WIDTH = dims[0];
        HEIGHT = dims[1];
        LENGTH = dims[2];
        ...
        if (WIDTH <= 0 || HEIGHT <= 0 || LENGTH <= 0) {
            ...
            if (error) {
                ...
                else { GETTER = new BoilerLiquidShape(shapes); }
            }
        }
    }
