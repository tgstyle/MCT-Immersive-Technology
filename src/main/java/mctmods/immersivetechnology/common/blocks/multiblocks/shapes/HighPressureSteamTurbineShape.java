...
public class HighPressureSteamTurbineShape extends GenericShape {
    public static final HighPressureSteamTurbineShape GETTER = new HighPressureSteamTurbineShape();
    ...
    static {
        ...
        try {
            InputStream is = HighPressureSteamTurbineShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/high_pressure_steam_turbine.json");
            if (is != null) {
                ...
                ITLogger.info("HighPressureSteamTurbineShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load HighPressureSteamTurbineShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("HighPressureSteamTurbineShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public HighPressureSteamTurbineShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 1, 0}); }
