...
public class ITBoilerLogic implements IMultiblockLogic<ITBoilerLogic.State>, IServerTickableComponent<ITBoilerLogic.State>, IClientTickableComponent<ITBoilerLogic.State> {
    ...
    public void tickClient(IMultiblockContext<ITBoilerLogic.State> ctx) {
        ...
        if (state.heatLevel == 0) { state.isSoundPlaying = () -> false; }
            ...
            if (!state.isSoundPlaying.getAsBoolean()) { state.isSoundPlaying = ITMultiblockSound.startSound(() -> state.heatLevel > 0, ctx.isValid(), soundPos, ITSounds.boiler, () -> { LocalPlayer player = Minecraft.getInstance().player; if (player == null) { return 0f; } float attenuation = (float) Math.max(player.distanceToSqr(soundPos) / 8, 1); float currentLevel = (float) (state.heatLevel / WORKING_HEAT_LEVEL); return (2 * currentLevel) / attenuation; }, () -> (float) (state.heatLevel / WORKING_HEAT_LEVEL)); }
        }
    }
    ...
    public void tickServer(IMultiblockContext<ITBoilerLogic.State> ctx) {
        ...
        if (!state.pilotLit) {
            ...
            if (state.burnRemaining > 0) {
                ...
                if (state.lastFuel != null) {
                    ...
                    FluidStack drained;
                    if (fullMode) {
                        int drainAmount = state.lastFuel.fuel.getAmount();
        ...
        if (state.heatLevel >= WORKING_HEAT_LEVEL) {
            if (state.recipeTimeRemaining > 0) {
                ...
                if (state.lastRecipe != null && state.lastRecipe.water.getAmount() <= state.tanks.input2.getFluidAmount() && state.lastRecipe.output.getAmount() <= state.tanks.output.getCapacity() - state.tanks.output.getFluidAmount()) {
                    ...
                    state.recipeTimeRemaining--;
                    if (state.recipeTimeRemaining == 0) { state.tanks.input2.drain(state.lastRecipe.water.getAmount(), FluidAction.EXECUTE); state.tanks.output.fill(state.lastRecipe.output.copy(), FluidAction.EXECUTE); }
                    update = true;
                }
            }
        }
            ...
            if (!emptyContainer.isEmpty()) {
                ...
                if (!filledContainer.isEmpty()) {
                    ...
                    if (outputStack.isEmpty() || (ItemHandlerHelper.canItemStacksStack(outputStack, filledContainer) && outputStack.getCount() + filledContainer.getCount() <= outputStack.getMaxStackSize())) {
                        ...
                        if (outputStack.isEmpty()) { state.inventory.setStackInSlot(OUTPUT_SLOT_FILLED, filledContainer); }
                        else { outputStack.grow(filledContainer.getCount()); }
                        update = true;
                    }
                }
            }
        ...
        if (!filledContainerFuel.isEmpty()) {
            ...
            if (!emptyContainer.isEmpty()) {
                ...
                if (outputStack.isEmpty() || (ItemHandlerHelper.canItemStacksStack(outputStack, emptyContainer) && outputStack.getCount() + emptyContainer.getCount() <= outputStack.getMaxStackSize())) {
                    ...
                    if (outputStack.isEmpty()) { state.inventory.setStackInSlot(INPUT_FUEL_SLOT_EMPTY, emptyContainer); }
                    else { outputStack.grow(emptyContainer.getCount()); }
                    update = true;
                }
            }
        }
        ...
        if (!filledContainerWater.isEmpty()) {
            ...
            if (!emptyContainer.isEmpty()) {
                ItemStack outputStack = state.inventory.getStackInSlot(INPUT_SLOT_EMPTY);
                if (outputStack.isEmpty() || (ItemHandlerHelper.canItemStacksStack(outputStack, emptyContainer) && outputStack.getCount() + emptyContainer.getCount() <= outputStack.getMaxStackSize())) {
                    ...
                    state.inventory.setStackInSlot(INPUT_SLOT_FILLED, filledContainerWater);
                    if (outputStack.isEmpty()) { state.inventory.setStackInSlot(INPUT_SLOT_EMPTY, emptyContainer); }
                    else { outputStack.grow(emptyContainer.getCount()); }
                    update = true;
                }
            }
        ...
        if (update) {
            ctx.markMasterDirty();
            if (state.clientUpdateCooldown == 1) { ctx.requestMasterBESync(); state.clientUpdateCooldown = 20; }
