...
public class CoolingTowerShape {
    public static final MultiblockData DATA = MultiblockDataLoader.loadMultiblockData("cooling_tower");
    public static final Function<BlockPos, VoxelShape> GETTER;
    public static int WIDTH, HEIGHT, LENGTH;

    static {
        int[] dims = GenericShape.loadDimensions("cooling_tower", "stone");
        ...
        if (WIDTH <= 0 || HEIGHT <= 0 || LENGTH <= 0) {
            ...
            if (DATA.shapeAABB == null) {
                ITLib.IT_LOGGER.error("Failed to load shapes for cooling_tower multiblock. (shapeAABB null)");
                GETTER = FullblockShape.GETTER;
            } else if (DATA.shapeAABB.isEmpty()) {
                ITLib.IT_LOGGER.info("Using full block shape for cooling_tower.");
                GETTER = FullblockShape.GETTER;
            } else {
                List<List<AABB>> shapes = GenericShape.loadShapes(DATA, num);
                if (shapes == null) {
                    ITLib.IT_LOGGER.error("Failed to load shapes for cooling_tower multiblock.");
                    GETTER = FullblockShape.GETTER;
                } else {
