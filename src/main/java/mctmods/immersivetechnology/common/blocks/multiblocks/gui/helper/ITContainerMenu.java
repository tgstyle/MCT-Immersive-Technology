...
import mctmods.immersivetechnology.common.blocks.helper.ITBlockEntity;
...
public abstract class ITContainerMenu extends AbstractContainerMenu {
    ...
    public void receiveSync(List<Pair<Integer, DataPair<?>>> synced) {
        for (Pair<Integer, DataPair<?>> syncElement : synced) genericData.get(syncElement.getFirst()).processSync(syncElement.getSecond().data());
    }
    ...
    public void clicked(int id, int dragType, @NotNull ClickType clickType, @NotNull Player player) {
        ...
        if (dragType == 2) { slot.set(ItemStack.EMPTY); }
            ...
            if (stackSlot.isEmpty()) {
                ...
                if (ItemStack.isSameItem(stackSlot, stackHeld)) stackSlot.grow(amount);
    ...
    public static void onContainerClosed(PlayerContainerEvent.Close ev) {
        if (ev.getContainer() instanceof ITContainerMenu itContainer && ev.getEntity() instanceof ServerPlayer serverPlayer) itContainer.usingPlayers.remove(serverPlayer);
    }
    ...
    public static MenuContext blockCtx(MenuType<?> pMenuType, int pContainerId, BlockEntity be) {
        return new MenuContext(pMenuType, pContainerId, () -> {
            be.setChanged();
            if (be instanceof ITBlockEntity itBE) itBE.markContainingBlockForUpdate(null);
        }, p -> {
            BlockPos pos = be.getBlockPos();
            Level level = be.getLevel();
            if (level == null || level.getBlockEntity(pos) != be) return false;
            else return !(p.distanceToSqr(pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D) > 64.0D);
        });
    }
    ...
    public static MenuContext itemCtx(MenuType<?> pMenuType, int pContainerId, Inventory playerInv, EquipmentSlot slot, ItemStack stack) {
        return new MenuContext(pMenuType, pContainerId, () -> {}, p -> {
            if (p != playerInv.player) return false;
