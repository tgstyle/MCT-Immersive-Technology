...
public class BoilerTankShape extends GenericShape {
    ...
    private BoilerTankShape(List<List<AABB>> shapes) { this.SHAPES = shapes; }

    static {
        int[] dims = loadDimensions("boiler_tank", "metal");
        WIDTH = dims[0];
        HEIGHT = dims[1];
        LENGTH = dims[2];
        ...
        if (WIDTH <= 0 || HEIGHT <= 0 || LENGTH <= 0) {
            ...
            if (error) {
                ...
                else { GETTER = new BoilerTankShape(shapes); }
            }
        }
    }
