...
public class SteelSheetmetalTankShape extends GenericShape {
    public static final SteelSheetmetalTankShape GETTER = new SteelSheetmetalTankShape();
    ...
    static {
        ...
        try {
            InputStream is = SteelSheetmetalTankShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/steel_sheetmetal_tank.json");
            if (is != null) {
                ...
                ITLogger.info("SteelSheetmetalTankShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load SteelSheetmetalTankShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("SteelSheetmetalTankShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public SteelSheetmetalTankShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 0, 0}); }
