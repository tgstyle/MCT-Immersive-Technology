...
public class TileEntityHighPressureSteamTurbineSlave extends TileEntityMultiblockNewSystem<TileEntityHighPressureSteamTurbineSlave, HighPressureSteamTurbineRecipe, TileEntityHighPressureSteamTurbineMaster> implements IAdvancedSelectionBounds, IAdvancedCollisionBounds, IMechanicalEnergy {

	public TileEntityHighPressureSteamTurbineSlave() {
		super(MultiblockHighPressureSteamTurbine.instance, new int[] { 4, 10, 3 }, 0, true);
	}
	...
	@Override
	protected HighPressureSteamTurbineRecipe readRecipeFromNBT(NBTTagCompound tag) { return HighPressureSteamTurbineRecipe.loadFromNBT(tag); }

	@Override
	public HighPressureSteamTurbineRecipe findRecipeForInsertion(ItemStack inserting) {
		return null;
	}
	...
	@Override
	public boolean additionalCanProcessCheck(MultiblockProcess <HighPressureSteamTurbineRecipe> process) {
		return false;
	}
	...
	@Override
	public void onProcessFinish(MultiblockProcess <HighPressureSteamTurbineRecipe> process) {
	}
	...
	@Override
	public float getMinProcessDistance(MultiblockProcess <HighPressureSteamTurbineRecipe> process) {
		return 0;
	}
	...
	protected IFluidTank[] getAccessibleFluidTanks(EnumFacing side) {
		TileEntityHighPressureSteamTurbineMaster master = master();
		if(master != null) {
	...
	protected boolean canFillTankFrom(int iTank, EnumFacing side, FluidStack resource) {
		TileEntityHighPressureSteamTurbineMaster master = this.master();
		if(master == null) return false;
		if(pos == 30 && side == facing.getOpposite()) {
			if(master.tanks[iTank].getFluidAmount() >= master.tanks[iTank].getCapacity()) return false;
			if(master.tanks[iTank].getFluid() == null) return HighPressureSteamTurbineRecipe.findFuelByFluid(resource.getFluid()) != null;
