...
public class TileEntityBarrel extends TileEntityCommonOSD implements IFluidTank, IPlayerInteraction, ITileDrop, IFluidTankProperties, IFluidHandler {
    ...
    public void update() {
        ...
        for (int index = 0; index < 6; index++) {
            ...
            if (output != null) {
                ...
                acceptedAmount += output.fill((tile instanceof ITIPipe)?infiniteFluidPressurized : infiniteFluid, true);
            }
        }
    }

    @Override
    public boolean hasCapability(@Nonnull final Capability<?> capability, final EnumFacing facing) { return capability == CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY; }
    ...
    @Override
    public FluidStack getFluid() { return infiniteFluid; }

    @Override
    public int getFluidAmount() { return Integer.MAX_VALUE; }

    @Nullable
    @Override
    public FluidStack getContents() { return infiniteFluid; }
    ...
    @Override
    public boolean canDrainFluidType(FluidStack fluidStack) { return fluidStack.getFluid() == this.infiniteFluid.getFluid(); }

    IFluidTankProperties[] tank = new IFluidTankProperties[] { this };

    @Override
    public FluidTankInfo getInfo() { return new FluidTankInfo(infiniteFluid, Integer.MAX_VALUE); }

    @Override
    public IFluidTankProperties[] getTankProperties() { return tank; }
    ...
    @Nullable
    @Override
    public FluidStack drain(FluidStack fluidStack, boolean b) { return this.drain(fluidStack.amount, b); }
    ...
    public void receiveMessageFromClient(ByteBuf buf, EntityPlayerMP player) {
        ...
        message.writeBoolean(infiniteFluid != null);
        if (infiniteFluid != null) ByteBufUtils.writeUTF8String(message, infiniteFluid.getFluid().getName());
    ...
    public void receiveMessageFromServer(ByteBuf buf) {
        ...
        if (hasFluid) setFluid(FluidRegistry.getFluid(ByteBufUtils.readUTF8String(buf)));
        else {
            infiniteFluid = null;
            infiniteFluidPressurized = null;
        }
