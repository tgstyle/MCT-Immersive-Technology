...
public class MultiblockHighPressureSteamTurbine implements IMultiblock {
	...
	public String getUniqueName() {
		return "IT:HighPressureSteamTurbine";
	}
	...
	public boolean createStructure(World world, BlockPos pos, EnumFacing side, EntityPlayer player) {
		...
		IBlockState master = ITContent.blockMetalMultiblock1.getStateFromMeta(BlockType_MetalMultiblock1.HIGH_PRESSURE_STEAM_TURBINE.getMeta());
		IBlockState slave = ITContent.blockMetalMultiblock1.getStateFromMeta(BlockType_MetalMultiblock1.HIGH_PRESSURE_STEAM_TURBINE_SLAVE.getMeta());
		...
		for(int h = - 1 ; h <= 2 ; h ++) {
			for(int l = 0 ; l <= 9 ; l ++) {
				for(int w = - 1 ; w <= 1 ; w ++) {
					...
					if(curr instanceof TileEntityHighPressureSteamTurbineSlave) {
						TileEntityHighPressureSteamTurbineSlave tile = (TileEntityHighPressureSteamTurbineSlave)curr;
						tile.facing = side;
						tile.formed = true;
