...
public class ConveyorExtractCoveredAlternative extends ConveyorExtractAlternative {
    ...
    @Override public List<BakedQuad> modifyQuads(List<BakedQuad> baseModel, @Nullable TileEntity tile, EnumFacing facing) {
        ...
        Function<Vector3f[], Vector3f[]> vertexTransformer = vertices -> {
            ...
            for (int i = 0; i < vertices.length; ++i) ret[i] = new Vector3f(vertices[i].x, vertices[i].y, vertices[i].z - extend);
            return ret;
        };
        Function<Vector3f[], Vector3f[]> casingTransformer = vertices -> {
            Vector3f[] ret = new Vector3f[vertices.length];
            for (int i = 0; i < vertices.length; ++i) ret[i] = new Vector3f(vertices[i].x, vertices[i].y - 0.25F, vertices[i].z - 0.625F - extend);
            return ret;
        };
        model.addAll(ClientUtils.createBakedBox(new Vector3f(0.0625F, 0.375F, 0.625F), new Vector3f(0.1875F, 1.0F, 1.0F), armMatrix, renderFacing, casingTransformer, getCasingSprite, colour));
        model.addAll(ClientUtils.createBakedBox(new Vector3f(0.8125F, 0.375F, 0.625F), new Vector3f(0.9375F, 1.0F, 1.0F), armMatrix, renderFacing, casingTransformer, getCasingSprite, colour));
        model.addAll(ClientUtils.createBakedBox(new Vector3f(0.1875F, 0.875F, 0.625F), new Vector3f(0.8125F, 1.0F, 1.0F), armMatrix, renderFacing, casingTransformer, getCasingSprite, colour));
        if (tile != null && extend > 0.0F) {
            TextureAtlasSprite tex_conveyor = ClientUtils.getSprite(isActive(tile) ? ConveyorBasicAlternative.texture_on : ConveyorBasicAlternative.texture_off);
            ...
            model.add(ClientUtils.createBakedQuad(DefaultVertexFormats.ITEM, ClientUtils.applyMatrixToVertices(armMatrix, vertices), Utils.rotateFacingTowardsDir(EnumFacing.DOWN, renderFacing), tex_conveyor, new double[]{15.0, extend * 16.0, 1.0, 0.0}, colour, true));
            for (Vector3f vec : vertices) vec.setY(0.125F);
            model.add(ClientUtils.createBakedQuad(DefaultVertexFormats.ITEM, ClientUtils.applyMatrixToVertices(armMatrix, vertices), Utils.rotateFacingTowardsDir(EnumFacing.UP, renderFacing), tex_conveyor, new double[]{15.0, (1.0F - extend) * 16.0, 1.0, 16.0}, colour, false));
            model.addAll(ClientUtils.createBakedBox(new Vector3f(0.0625F, 0.25F, 0.625F), new Vector3f(0.9375F, 0.375F, 0.625F + extend), armMatrix, renderFacing, casingTransformer, getExtensionSprite, colour));
        }
        ...
        model.add(ClientUtils.createBakedQuad(DefaultVertexFormats.ITEM, ClientUtils.applyMatrixToVertices(armMatrix, vertexTransformer.apply(vertices)), Utils.rotateFacingTowardsDir(EnumFacing.NORTH, renderFacing), texture_assembler, new double[]{15.25, 13.25, 12.75, 15.25}, colour, false));
        for (Vector3f vec : vertices) vec.setZ(0.0625F);
        model.add(ClientUtils.createBakedQuad(DefaultVertexFormats.ITEM, ClientUtils.applyMatrixToVertices(armMatrix, vertexTransformer.apply(vertices)), Utils.rotateFacingTowardsDir(EnumFacing.SOUTH, renderFacing), texture_assembler, new double[]{12.75, 13.25, 15.25, 15.25}, colour, true));
        for (int i = 0; i < 5; ++i) {
            float off = i * 0.125F;
            model.addAll(ClientUtils.createBakedBox(new Vector3f(0.203125F + off, 0.1875F, 0.09375F), new Vector3f(0.296875F + off, 0.625F, 0.125F), armMatrix, renderFacing, vertexTransformer, f -> texture_curtain, colour));
        }
