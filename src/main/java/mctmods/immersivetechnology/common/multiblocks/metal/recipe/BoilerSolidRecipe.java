...
public class BoilerSolidRecipe extends MultiblockRecipe {
    ...
    public BoilerSolidRecipe(ResourceLocation id, IngredientWithSize input, double heatPerTick) {
    ...
    public static BoilerSolidRecipe findRecipe(Level level, ItemStack input) {
        for (BoilerSolidRecipe recipe : RECIPES.getRecipes(level)) { if (recipe.input.testIgnoringSize(input)) return recipe; }
        return null;
    }
