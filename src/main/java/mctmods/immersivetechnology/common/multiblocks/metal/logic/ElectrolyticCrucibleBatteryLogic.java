...
public class ElectrolyticCrucibleBatteryLogic implements IMultiblockLogic<ElectrolyticCrucibleBatteryLogic.State>, IServerTickableComponent<ElectrolyticCrucibleBatteryLogic.State>, IClientTickableComponent<ElectrolyticCrucibleBatteryLogic.State>, ITPressurizedFluidOutput<ElectrolyticCrucibleBatteryLogic.State> {
    ...
    private static final List<BlockPos> FLUID_OUTPUT_POIS = ImmutableList.of(
            safeGetPos("fluid_output0"),
            safeGetPos("fluid_output1"),
            safeGetPos("fluid_output2")
    );
    ...
    private static final List<RelativeBlockFace> OUTPUT_FACINGS = ImmutableList.of(
            getFacingSafe("fluid_output0"),
            getFacingSafe("fluid_output1"),
            getFacingSafe("fluid_output2")
    );

    private static List<BlockPos> getPosList(String name) {
        return RAW_POIS.stream()
                .filter(poi -> poi.name.equals(name))
                .map(poi -> new BlockPos(poi.pos[0], poi.pos[1], poi.pos[2]))
                .collect(ImmutableList.toImmutableList());
    }

    private static BlockPos safeGetPos(String name) {
        List<BlockPos> list = getPosList(name);
        return list.isEmpty() ? BlockPos.ZERO : list.get(0);
    }

    private static RelativeBlockFace getFacingSafe(String name) {
        try {
            List<RelativeBlockFace> facings = RAW_POIS.stream()
                    .filter(poi -> poi.name.equals(name))
            ...
            return RelativeBlockFace.FRONT;
        }
    }

    private static CapabilityPosition safeGetCapabilityPos(String name) {
        BlockPos pos = safeGetPos(name);
        RelativeBlockFace facing = getFacingSafe(name);
        return new CapabilityPosition(pos, facing);
    }

    private static List<CapabilityPosition> getEnergyPOIs() {
        return RAW_POIS.stream()
                .filter(poi -> poi.name.startsWith("energy_input"))
                .map(poi -> safeGetCapabilityPos(poi.name))
                .collect(ImmutableList.toImmutableList());
    }
    ...
    @Override
    public List<RelativeBlockFace> getOutputFacings() { return OUTPUT_FACINGS; }
    ...
    public void tickClient(IMultiblockContext<State> ctx) {
        ...
        if (state.active && vol > 0.01f && !state.isSoundPlaying.getAsBoolean()) {
            state.isSoundPlaying = ITSound.startSound(
                    () -> state.active, ctx.isValid(), soundPos, ITSounds.electrolyticCrucibleBattery,
                    () -> {
    ...
    public void tickServer(IMultiblockContext<State> ctx) {
        ...
        ElectrolyticCrucibleBatteryRecipe recipe = ElectrolyticCrucibleBatteryRecipe.findRecipe(ctx.getLevel().getRawLevel(), state.tanks.input.getFluid());
        ...
        if (!itemOutput.isEmpty()) {
            ...
            inventory.setStackInSlot(0, itemOutput);
        }
    ...
    public <T> LazyOptional<T> getCapability(IMultiblockContext<State> ctx, CapabilityPosition position, Capability<T> cap) {
        ...
        if (cap == ForgeCapabilities.ENERGY) {
            for (CapabilityPosition poi : ENERGY_POIS) {
                if (position.posInMultiblock().equals(poi.posInMultiblock()) && (position.side() == null || position.side() == poi.side())) { return state.energyCap.cast(ctx); }
            }
            ...
            if (position.posInMultiblock().equals(OUTPUT_FLUID_POI_0.posInMultiblock()) && (position.side() == null || position.side() == OUTPUT_FLUID_POI_0.side())) { return state.outputCap0.cast(ctx); }
            if (position.posInMultiblock().equals(OUTPUT_FLUID_POI_1.posInMultiblock()) && (position.side() == null || position.side() == OUTPUT_FLUID_POI_1.side())) { return state.outputCap1.cast(ctx); }
            if (position.posInMultiblock().equals(OUTPUT_FLUID_POI_2.posInMultiblock()) && (position.side() == null || position.side() == OUTPUT_FLUID_POI_2.side())) { return state.outputCap2.cast(ctx); }
    ...
    public Function<BlockPos, VoxelShape> shapeGetter(ShapeType shapeType) {
        return ElectrolyticCrucibleBatteryShape.GETTER;
    }
    ...
    public static class State implements IMultiblockState, ITProcessContext.ProcessContextInMachine<ElectrolyticCrucibleBatteryRecipe>, ITDisplayContext {
        ...
        public State(IInitialMultiblockContext<State> ctx) {
            ...
            inventory = new ITSlotwiseItemHandler(
                    List.of(ITSlotwiseItemHandler.IOConstraint.OUTPUT),
                    onChanged
            );
    ...
    public record ElectrolyticCrucibleBatteryTanks(ITMarkableFluidTank input, ITMarkableFluidTank output0, ITMarkableFluidTank output1, ITMarkableFluidTank output2) {
        public ElectrolyticCrucibleBatteryTanks(Consumer<Void> markDirty) {
            this(
                    new ITMarkableFluidTank(TANK_CAPACITY, markDirty),
                    new ITMarkableFluidTank(TANK_CAPACITY, markDirty),
                    new ITMarkableFluidTank(TANK_CAPACITY, markDirty),
                    new ITMarkableFluidTank(TANK_CAPACITY, markDirty)
            );
