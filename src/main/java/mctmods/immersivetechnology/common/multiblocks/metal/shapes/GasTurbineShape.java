...
public class GasTurbineShape extends GenericShape {
    ...
    static {
        ...
        try {
            InputStream is = GasTurbineShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/gas_turbine.json");
            if (is != null) {
                ...
                ITLogger.info("GasTurbineShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load GasTurbineShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("GasTurbineShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public GasTurbineShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 1, 0}); }
