...
public class SolarTowerShape extends GenericShape {
    ...
    static {
        ...
        try {
            InputStream is = SolarTowerShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/solar_tower.json");
            if (is != null) {
                ...
                ITLogger.info("SolarTowerShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load SolarTowerShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("SolarTowerShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public SolarTowerShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 1, 0}); }
