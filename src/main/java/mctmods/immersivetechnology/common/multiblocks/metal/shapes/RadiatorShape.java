...
public class RadiatorShape extends GenericShape {
    public static RadiatorShape GETTER = new RadiatorShape();
    ...
    static {
        ...
        try {
            InputStream is = RadiatorShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/radiator.json");
            if (is != null) {
                ...
                ITLogger.info("RadiatorShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load RadiatorShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("RadiatorShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public RadiatorShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 3, 0}); }
