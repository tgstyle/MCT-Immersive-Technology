...
public class HeatExchangerShape {
    ...
    static {
        int[] dims = GenericShape.loadDimensions("heat_exchanger", "metal");
        WIDTH = dims[0] + DATA.padShape[0];
        HEIGHT = dims[1] + DATA.padShape[1];
        LENGTH = dims[2] + DATA.padShape[2];
        ITLib.IT_LOGGER.info("Loaded dimensions for heat_exchanger: W={}, H={}, L={}", WIDTH, HEIGHT, LENGTH);
        if (WIDTH <= 0 || HEIGHT <= 0 || LENGTH <= 0) {
            GETTER = FullblockShape.GETTER;
            WIDTH = HEIGHT = LENGTH = 0;
            if (DATA.shapeAABB == null || !DATA.shapeAABB.isEmpty()) {
                ITLib.IT_LOGGER.error("Invalid dimensions loaded for heat_exchanger multiblock.");
            }
        } else {
            int num = WIDTH * HEIGHT * LENGTH;
            if (DATA.shapeAABB == null) {
                ITLib.IT_LOGGER.error("Failed to load shapes for heat_exchanger multiblock. (shapeAABB null)");
                GETTER = FullblockShape.GETTER;
            } else if (DATA.shapeAABB.isEmpty()) {
                ITLib.IT_LOGGER.info("Using full block shape for heat_exchanger.");
                GETTER = FullblockShape.GETTER;
            } else {
                List<List<AABB>> shapes = GenericShape.loadShapes(DATA, num);
                if (shapes == null) {
                    ITLib.IT_LOGGER.error("Failed to load shapes for heat_exchanger multiblock.");
                    GETTER = FullblockShape.GETTER;
                } else {
