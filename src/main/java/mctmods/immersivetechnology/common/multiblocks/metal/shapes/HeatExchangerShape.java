...
public class HeatExchangerShape extends GenericShape {
    public static final HeatExchangerShape GETTER = new HeatExchangerShape();
    ...
    static {
        ...
        try {
            InputStream is = HeatExchangerShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/heat_exchanger.json");
            if (is != null) {
                ...
                ITLogger.info("HeatExchangerShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load HeatExchangerShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("HeatExchangerShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public HeatExchangerShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 1, 0}); }
