...
public class DistillerShape extends GenericShape {
    ...
    static {
        ...
        try {
            InputStream is = DistillerShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/distiller.json");
            if (is != null) {
                ...
                ITLogger.info("DistillerShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load DistillerShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("DistillerShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public DistillerShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 1, 0}); }
