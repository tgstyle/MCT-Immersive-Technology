...
public class CoolingTowerShape extends GenericShape {
    ...
    static {
        ...
        try {
            InputStream is = CoolingTowerShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/cooling_tower.json");
            if (is != null) {
                ...
                ITLogger.info("CoolingTowerShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load CoolingTowerShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("CoolingTowerShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public CoolingTowerShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 1, 0}); }
