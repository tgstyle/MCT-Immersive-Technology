...
public class ElectrolyticCrucibleBatteryProcess extends MultiblockProcessInMachine<ElectrolyticCrucibleBatteryRecipe> {
    ...
    @Override
    public void doProcessTick(ProcessContext.ProcessContextInMachine<ElectrolyticCrucibleBatteryRecipe> context, IMultiblockLevel level) {
        ...
        if (this.processTick == 0) {
            ...
            if (drained.getAmount() < amount || !levelData.recipe().fluidInput0.testIgnoringAmount(drained)) {
                this.clearProcess = true;
                return;
            }
    ...
    @Override
    public boolean canProcess(ProcessContext.ProcessContextInMachine<ElectrolyticCrucibleBatteryRecipe> context, Level level) {
    ...
    @Override
    protected void processFinish(ProcessContext.ProcessContextInMachine<ElectrolyticCrucibleBatteryRecipe> context, IMultiblockLevel level) {
        ...
        if (recipe.fluidOutput0 != null && !recipe.fluidOutput0.isEmpty()) {
            state.getTanks().output0().fill(recipe.fluidOutput0.copy(), FluidAction.EXECUTE);
        }
        if (recipe.fluidOutput1 != null && !recipe.fluidOutput1.isEmpty()) {
            state.getTanks().output1().fill(recipe.fluidOutput1.copy(), FluidAction.EXECUTE);
        }
        if (recipe.fluidOutput2 != null && !recipe.fluidOutput2.isEmpty()) {
            state.getTanks().output2().fill(recipe.fluidOutput2.copy(), FluidAction.EXECUTE);
        }
    }
}
