...
public class ElectrolyticCrucibleBatteryShape {
    public static final MultiblockData DATA = MultiblockDataLoader.loadMultiblockData("electrolytic_crucible_battery");
    ...
    static {
        int[] dims = GenericShape.loadDimensions("electrolytic_crucible_battery", "metal");
        WIDTH = dims[0] + DATA.padShape[0];
        HEIGHT = dims[1] + DATA.padShape[1];
        LENGTH = dims[2] + DATA.padShape[2];
        ITLib.IT_LOGGER.info("Loaded dimensions for electrolytic_crucible_battery: W={}, H={}, L={}", WIDTH, HEIGHT, LENGTH);
