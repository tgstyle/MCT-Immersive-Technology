...
public class ElectrolyticCrucibleBatteryShape extends GenericShape {
    public static final ElectrolyticCrucibleBatteryShape GETTER = new ElectrolyticCrucibleBatteryShape();
    ...
    static {
        ...
        try {
            InputStream is = ElectrolyticCrucibleBatteryShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/electrolytic_crucible_battery.json");
            if (is != null) {
                ...
                ITLogger.info("ElectrolyticCrucibleBatteryShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load ElectrolyticCrucibleBatteryShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("ElectrolyticCrucibleBatteryShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public ElectrolyticCrucibleBatteryShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 1, 0}); }
