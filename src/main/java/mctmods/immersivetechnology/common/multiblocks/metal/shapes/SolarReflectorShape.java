...
public class SolarReflectorShape extends GenericShape {
    public static final SolarReflectorShape GETTER = new SolarReflectorShape();
    ...
    static {
        ...
        try {
            InputStream is = SolarReflectorShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/solar_reflector.json");
            if (is != null) {
                ...
                ITLogger.info("SolarReflectorShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load SolarReflectorShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("SolarReflectorShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public SolarReflectorShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 1, 0}); }
