...
public class SolarReflectorShape {
    ...
    static {
        int[] dims = GenericShape.loadDimensions("solar_reflector", "metal");
        ...
        if (WIDTH <= 0 || HEIGHT <= 0 || LENGTH <= 0) {
            ...
            if (DATA.shapeAABB == null) {
                ITLib.IT_LOGGER.error("Failed to load shapes for solar_reflector multiblock. (shapeAABB null)");
                GETTER = FullblockShape.GETTER;
            } else if (DATA.shapeAABB.isEmpty()) {
                ITLib.IT_LOGGER.info("Using full block shape for solar_reflector.");
                GETTER = FullblockShape.GETTER;
            } else {
