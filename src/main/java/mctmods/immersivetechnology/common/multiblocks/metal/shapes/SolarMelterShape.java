...
public class SolarMelterShape extends GenericShape {
    public static SolarMelterShape GETTER = new SolarMelterShape();
    ...
    static {
        ...
        try {
            InputStream is = SolarMelterShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/solar_melter.json");
            if (is != null) {
                ...
                ITLogger.info("SolarMelterShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load SolarMelterShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("SolarMelterShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public SolarMelterShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{1, 3, 1}); }
