...
public interface ITIPressurizedFluidOutput<State extends IMultiblockState> {
    ...
    default boolean pumpOutputs(IMultiblockContext<State> ctx) {
        ...
        java.util.Map<ITMarkableFluidTank, List<Integer>> byTank = new java.util.LinkedHashMap<>();
        for (int i = 0; i < tanks.size(); i++) { byTank.computeIfAbsent(tanks.get(i), k -> new java.util.ArrayList<>()).add(i); }

        for (java.util.Map.Entry<ITMarkableFluidTank, List<Integer>> entry : byTank.entrySet()) {
            ITMarkableFluidTank tank = entry.getKey();
            ...
            for (int idx = 0; idx < connected.size(); idx++) {
                ...
                fs = ITUtils.copyFluidStackWithAmount(fs, Math.min(share, fs.getAmount()), false);
