...
public class ClientMultiblockProperties implements ClientMultiblocks.MultiblockManualData {
    private final ModTemplateMultiblock multiblock;
    @Nullable private NonNullList<ItemStack> materials;
    private final ItemStack renderStack;
    @Nullable private final Vec3 renderOffset;

    public ClientMultiblockProperties(ModTemplateMultiblock multiblock, double offX, double offY, double offZ) { this(multiblock, new Vec3(offX, offY, offZ)); }

    private ClientMultiblockProperties(ModTemplateMultiblock multiblock, @Nullable Vec3 renderOffset) {
    ...
    @Override public NonNullList<ItemStack> getTotalMaterials() {
        if (this.materials == null) {
            ...
            for (StructureTemplate.StructureBlockInfo info : structure) {
                if (info.state().hasProperty(ModProperties.MULTIBLOCKSLAVE) && info.state().getValue(ModProperties.MULTIBLOCKSLAVE)) continue;
                ItemStack picked = ClientUtils.getPickBlock(info.state());
                boolean added = false;
