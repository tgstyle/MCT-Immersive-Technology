...
public class AdvancedCokeOvenShape extends GenericShape {
    ...
    static {
        ...
        try {
            InputStream is = AdvancedCokeOvenShape.class.getResourceAsStream("/assets/immersivetech/multiblocks/advanced_coke_oven.json");
            if (is != null) {
                ...
                ITLogger.info("AdvancedCokeOvenShape loaded: SHAPES size=" + rawShapes.size() + ", master pos=" + masterPos);
            }
        } catch (Exception e) {
            ITLogger.error("Failed to load AdvancedCokeOvenShape: " + e.getMessage(), e);
        }
        SHAPES = rawShapes;
        MASTER_GRID_POS = masterPos;
        if(FMLCommonHandler.instance().getSide().isClient()) ITLogger.info("AdvancedCokeOvenShape loaded on client: SHAPES size=" + rawShapes.size());
    }

    public AdvancedCokeOvenShape() { super(WIDTH, HEIGHT, LENGTH, new int[]{0, 1, 0}); }
