...
public abstract class ContainerMenu extends AbstractContainerMenu {
    ...
    @Override public void broadcastChanges() {
        ...
        if (!toSync.isEmpty()) for (ServerPlayer player : usingPlayers) { PacketHandler.sendToPlayer(player, new MessageContainerData(toSync)); }
    }

    public void receiveSync(List<Pair<Integer, GenericDataSerializers.DataPair<?>>> synced) {
        for (Pair<Integer, GenericDataSerializers.DataPair<?>> syncElement : synced) { genericData.get(syncElement.getFirst()).processSync(syncElement.getSecond().data()); }
    }
    ...
    @SubscribeEvent public static void onContainerOpened(PlayerContainerEvent.Open ev) {
        if (ev.getContainer() instanceof ContainerMenu itContainer && ev.getEntity() instanceof ServerPlayer serverPlayer) {
            ...
            for (int i = 0; i < itContainer.genericData.size(); i++) {
                GenericContainerData<?> data = itContainer.genericData.get(i);
                data.needsUpdate(); // force `current` to be populated with the live value before serializing it
                list.add(Pair.of(i, data.dataPair()));
            }
            PacketHandler.sendToPlayer(serverPlayer, new MessageContainerData(list));
        }
    }
    ...
    public static MenuContext blockCtx(MenuType<?> pMenuType, int pContainerId, BlockEntity be) {
        return new MenuContext(pMenuType, pContainerId, () -> {
            be.setChanged();
            if (be instanceof BaseBlockEntity itBE) itBE.markContainingBlockForUpdate(null);
        }, p -> {
