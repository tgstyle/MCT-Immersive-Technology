...
public abstract class ITAbstractSplitModel<T extends BakedModel> extends CompositeBakedModel<T> {
    ...
    @Override @Nonnull public ModelData getModelData(@NotNull BlockAndTintGetter world, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull ModelData tileData) {
        ...
        if (te instanceof ITIModelOffsetProvider offsetProvider) { offset = offsetProvider.getModelOffset(state, size); } else if (state.getBlock() instanceof ITIModelOffsetProvider offsetProvider) { offset = offsetProvider.getModelOffset(state, size); }
        if (offset != null) { return baseData.derive().with(Model.SUBMODEL_OFFSET, offset).build(); } else { return baseData; }
    }
    ...
    protected Map<Vec3i, List<BakedQuad>> split(List<BakedQuad> in, Set<Vec3i> parts, ModelState transform) {
        ...
        for (var e : clumpedModel.getClumpedParts().entrySet()) {
            ...
            for (ITPolygon<ITPolygonUtils.ExtraQuadData> p : e.getValue().getFaces()) { subModelFaces.add(ITPolygonUtils.toBakedQuad(p.getPoints(), p.getTexture(), rotation, true)); }
