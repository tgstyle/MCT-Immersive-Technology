...
public class ITBakedDynamicSplitModel<K, T extends ICacheKeyProvider<K> & BakedModel> extends ITAbstractSplitModel<T> {
    private static final Set<ITBakedDynamicSplitModel<?, ?>> WEAK_INSTANCES = Collections.newSetFromMap(new WeakHashMap<>());
    static { IEApi.renderCacheClearers.add(() -> WEAK_INSTANCES.forEach(m -> m.subModelCache.invalidateAll())); }
