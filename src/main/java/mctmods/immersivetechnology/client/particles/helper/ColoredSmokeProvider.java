...
public record ColoredSmokeProvider(SpriteSet sprites) implements ParticleProvider<ColoredSmoke> {
    @Override public Particle createParticle(ColoredSmoke data, @NotNull ClientLevel level, double x, double y, double z, double velX, double velY, double velZ) {
        return new ModColoredSmoke(level, x, y, z, velX, velY, velZ, data.color, data.collideHorizontal, data.collideVertical, this.sprites);
    }
}
