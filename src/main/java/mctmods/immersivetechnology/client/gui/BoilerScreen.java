...
public class BoilerScreen extends ITContainerScreen<BoilerMenu> {
    private static final ResourceLocation TEXTURE = ITLib.makeTextureLocation("boiler");

    public BoilerScreen(BoilerMenu container, Inventory inventoryPlayer, Component title) { super(container, inventoryPlayer, title, TEXTURE); }
    ...
    protected void drawContainerBackgroundPre(@Nonnull GuiGraphics graphics, float partialTicks, int mouseX, int mouseY) {
        ...
        graphics.blit(TEXTURE, leftPos + 100, topPos + 5, 176, 0, barWidth, 9);
    }
    ...
    protected List<ITInfoArea> makeInfoAreas() {
        return ImmutableList.of(
                new ITFluidInfoArea(menu.tanks.input1(), new Rect2i(leftPos + 13, topPos + 20, 16, 47), 177, 31, 20, 51, TEXTURE),
                new ITFluidInfoArea(menu.tanks.input2(), new Rect2i(leftPos + 100, topPos + 20, 16, 47), 177, 31, 20, 51, TEXTURE),
                new ITFluidInfoArea(menu.tanks.output(), new Rect2i(leftPos + 125, topPos + 20, 16, 47), 177, 31, 20, 51, TEXTURE),
                new ITInfoArea(new Rect2i(leftPos + 100, topPos + 5, 41, 9)) {
                    @Override
