...
def intersects_box_vec(triangles, voxel_centers, box_extents):
    ...
    normal = torch.stack([normal_x, normal_y, normal_z], dim=-1).unsqueeze(0)
...
def moller_trumbore(ray_o: torch.Tensor, ray_d: torch.Tensor, tris: torch.Tensor, eps=1e-8) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    E1 = tris[:, 1] - tris[:, 0] # (n_faces, 3)
    E2 = tris[:, 2] - tris[:, 0] # (n_faces, 3)
    N = torch.cross(E1, E2) # (n_faces, 3)
    invdet = 1. / -(torch.einsum('md,nd->mn', ray_d, N) + eps) # (n_rays, n_faces)
    A0 = ray_o[:, None] - tris[None, :, 0] # (n_rays, n_faces, 3)
    DA0 = torch.cross(A0, ray_d[:, None].expand(*A0.shape)) # (n_rays, n_faces, 3)
    u = torch.einsum('mnd,nd->mn', DA0, E2) * invdet
    v = -torch.einsum('mnd,nd->mn', DA0, E1) * invdet
    t = torch.einsum('mnd,nd->mn', A0, N) * invdet # t > 0 for ray
    return u, v, t
...
def mark_surface(bx, by, bz, triangles, res, extents_base, voxel_size, is_mirrored, global_filled, size_x, size_y, size_z):
    ...
    if is_mirrored: filled = filled[:, :, ::-1]
...
parser.add_argument('--ray_fill', type=bool, default=False, help='Use ray casting for interior fill instead of flood fill (default: False)')
...
if args.fill_interior:
    if args.ray_fill:
        all_tris_list = triangles
        if mirrored_triangles is not None:
            all_tris_list += mirrored_triangles
        for _, _, _, add_tris, _ in additional_triangles:
            all_tris_list += add_tris
        if all_tris_list:
            all_tris = np.stack(all_tris_list)
            all_tris_t = torch.from_numpy(all_tris).to(device, torch.float64)
            gx, gy, gz = np.where(~global_filled)
            if len(gx) > 0:
                centers = np.stack([gx, gy, gz], axis=-1).astype(np.float64) * voxel_size + voxel_size / 2
                centers_t = torch.from_numpy(centers).to(device, torch.float64)
                dir_vec = torch.tensor([1.0, 1.0, 1.0], device=device, dtype=torch.float64)
                dir_vec /= dir_vec.norm()
                ray_d = dir_vec.unsqueeze(0).repeat(len(centers), 1)
                u, v, t = moller_trumbore(centers_t, ray_d, all_tris_t)
                mask = (u >= 0) & (v >= 0) & (u + v <= 1) & (t > EPSILON)
                counts = mask.sum(dim=1)
                inside = (counts % 2 == 1)
                inside_indices = np.where(inside.cpu().numpy())[0]
                for idx in inside_indices:
                    global_filled[gx[idx], gy[idx], gz[idx]] = True
    else:
            ...
            for gy in range(global_shape[1]):
                for gz in range(global_shape[2]):
                    if not visited[gx, gy, gz] and not global_filled[gx, gy, gz]:
                        global_filled[gx, gy, gz] = True
# Remove small connected components if min_vox > 1
if args.min_vox > 1:
    visited = np.zeros(global_shape, dtype=bool)
